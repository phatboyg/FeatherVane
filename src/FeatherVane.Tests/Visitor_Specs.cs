namespace FeatherVane.Tests
{
    ...
    public class Visiting_a_set_of_vanes
    {
        ...
        public void Should_display_the_types_of_all_the_vanes()
        {
            ...
            Vane<TestSubject> wired = Vane.Success<TestSubject>();

            var typeRouter = new TypeRouter<TestSubject>(x => typeof(ISubjectA));
            typeRouter.Add(Vane.Success<ISubjectA>(), x => x);
            typeRouter.Add(Vane.Success<ISubjectB>(), x => x);

            Vane<TestSubject> vane = Vane.Success(new Logger<TestSubject>(sw, x => ""),
