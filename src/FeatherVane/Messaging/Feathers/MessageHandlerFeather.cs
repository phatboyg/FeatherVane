...
namespace FeatherVane.Messaging.Feathers
{
    ...
    public class MessageHandlerFeather<T> :
        Feather<Message<T>>
        where T : class
    {
        readonly Action<Payload, Message<T>> _handlerMethod;

        public MessageHandlerFeather(Action<Payload, Message<T>> handlerMethod)
        {
            _handlerMethod = handlerMethod;
        }

        void Feather<Message<T>>.Compose(Composer composer, Payload<Message<T>> payload,
            Vane<Message<T>> next)
        {
