...
namespace FeatherVane.Messaging.Vanes
{
    ...
    public class MessageConsumerVane<T, TConsumer> :
        FeatherVane<Tuple<Message<T>, TConsumer>>
        where T : class
    {
        readonly Func<TConsumer, Action<Payload, Message<T>>> _selector;

        public MessageConsumerVane(Func<TConsumer, Action<Payload, Message<T>>> selector)
        {
            _selector = selector;
        }
